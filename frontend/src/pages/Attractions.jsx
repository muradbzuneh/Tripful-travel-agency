...
export default function Attractions() {
  ...
  const handleSearch = async () => {
    ...
    const filteredAttractions = attraction.filter(
      (attr) =>
    ...
    try {
      ...
      const related = packages.filter(
        (pkg) =>
          pkg.destination
            .toLowerCase()
            .includes(attraction.place.toLowerCase()) ||
          pkg.title.toLowerCase().includes(attraction.place.toLowerCase()) ||
          (pkg.location &&
            pkg.location.toLowerCase().includes(attraction.place.toLowerCase()))
      );
      setRelatedPackages(related);
    } catch (error) {
      console.error("Failed to fetch packages:", error);
    ...
    <div className="attractions-page">
      <div className="container">
        <div className="page-header">
          ...
          <p>
            Discover the rich history and cultural heritage of Ethiopia's most
          ...
          {attractions.map((attr) => (
            ...
            >
              ...
              <div className="attraction-content">
                ...
                <h4>📜 History</h4>
                <p>{attr.History}</p>
                <button className="explore-btn">Explore {attr.place}</button>
              </div>
            </div>
          ))}
        </div>
          ...
          <div className="selected-attraction">
            ...
            {relatedPackages.length > 0 && (
              <div className="related-packages">
                ...
                <p>
                  We found {relatedPackages.length} package(s) that include{" "}
                ...
                <div className="packages-suggestions">
                  {relatedPackages.map((pkg) => (
                    <div key={pkg.id} className="package-suggestion">
                      ...
                      <p>📍 {pkg.destination}</p>
                      <p>
                        💰 ${pkg.price} for {pkg.duration_days} days
                      </p>
                      <Link
                        to={`/package/${pkg.id}`}
        ...
        {attractions.length === 0 && searchTerm && (
          <div className="no-results">
            ...
            <p>
              Try searching with different keywords or browse our default
              attractions above.
            </p>
            <button
              onClick={() => {
        ...
        <div className="cta-section">
          ...
          <p>
            Check out our complete travel packages that include these amazing
            attractions!
          </p>
